    ...
    routes.get('/',(request, respond)=>{
        try{
            category.getAll().then((categories)=>{
                return respond.status(200).send({success:true,message:'categories successfully fetched',error:null,data:categories})
            }).catch((e)=>{
                return respond.status(200).send({success:false,message:'Unable to fetch categories',error:e,data:null})
            })
        }catch(e){
    ...
    routes.post('/add/masterCategory',jwtMiddleware,checkAdminPermissions,(request, respond)=>{
        try{
            ...
            category.addNewMasterCategory(masterCategoryName).then((products)=>{
                return respond.status(200).send({success:true,message:'Master category successfully added',error:null,data:products})
            }).catch((e)=>{
                return respond.status(200).send({success:false,message:'Unable to add category',error:e,data:null})
            })
        }catch(e){
            return respond.status(500).send({success:false,message:'Unexpected error occurs',error:e.message,data:null})
        }
